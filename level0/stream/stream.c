...
__global__ void tuned_STREAM_Add_cuda(STREAM_TYPE *d_a, STREAM_TYPE *d_b,
                                      STREAM_TYPE *d_c);
__global__ void tuned_STREAM_Triad_cuda(STREAM_TYPE *d_a, STREAM_TYPE *d_b,
                                        STREAM_TYPE *d_c);
...
// sycl::queue sycl_q(sycl::property::queue::in_order()); // persistent SYCL
// queue
sycl::queue sycl_q; // persistent SYCL queue

#ifdef USE_SYCL_ESIMD
...
#endif
static_assert(STREAM_ARRAY_SIZE % (2 * VL) == 0,
              "STREAM_ARRAY_SIZE must be a multiple of 2*VL");

void tuned_STREAM_Copy_sycl(sycl::queue &q, STREAM_TYPE *d_a,
                            STREAM_TYPE *d_c) {
  q.submit([&](sycl::handler &h) {
    h.parallel_for<class StreamCopyESIMD>(
        sycl::nd_range<1>(sycl::range<1>{STREAM_ARRAY_SIZE / (2 * VL)},
                          sycl::range<1>{64}),
...
void tuned_STREAM_Scale_sycl(sycl::queue &q, STREAM_TYPE *d_c,
                             STREAM_TYPE *d_b) {
  q.submit([&](sycl::handler &h) {
...
void tuned_STREAM_Add_sycl(sycl::queue &q, STREAM_TYPE *d_a, STREAM_TYPE *d_b,
                           STREAM_TYPE *d_c) {
  q.submit([&](sycl::handler &h) {
...
void tuned_STREAM_Triad_sycl(sycl::queue &q, STREAM_TYPE *d_a, STREAM_TYPE *d_b,
                             STREAM_TYPE *d_c) {
  q.submit([&](sycl::handler &h) {
...
void tuned_STREAM_Copy_sycl(sycl::queue &q, STREAM_TYPE *d_a,
                            STREAM_TYPE *d_c) {
  q.parallel_for(sycl::range<1>(STREAM_ARRAY_SIZE),
                 [=](sycl::id<1> i) { d_c[i] = d_a[i]; });
}

void tuned_STREAM_Scale_sycl(sycl::queue &q, STREAM_TYPE *d_c,
                             STREAM_TYPE *d_b) {
  q.parallel_for(sycl::range<1>(STREAM_ARRAY_SIZE),
                 [=](sycl::id<1> i) { d_b[i] = 3.0 * d_c[i]; });
}
...
void tuned_STREAM_Triad_sycl(sycl::queue &q, STREAM_TYPE *d_a, STREAM_TYPE *d_b,
                             STREAM_TYPE *d_c) {
  q.parallel_for(sycl::range<1>(STREAM_ARRAY_SIZE),
                 [=](sycl::id<1> i) { d_a[i] = d_b[i] + 3.0 * d_c[i]; });
}
#endif
#endif
...
int main() {
  ...
  posix_memalign((void **)&a, 2097152,
                 ((size_t)STREAM_ARRAY_SIZE) * sizeof(STREAM_TYPE));
  posix_memalign((void **)&b, 2097152,
                 ((size_t)STREAM_ARRAY_SIZE) * sizeof(STREAM_TYPE));
  posix_memalign((void **)&c, 2097152,
                 ((size_t)STREAM_ARRAY_SIZE) * sizeof(STREAM_TYPE));
  ...
  cudaHostAlloc((void **)&a, ((size_t)STREAM_ARRAY_SIZE) * sizeof(STREAM_TYPE),
                cudaHostAllocMapped);
  cudaHostAlloc((void **)&b, ((size_t)STREAM_ARRAY_SIZE) * sizeof(STREAM_TYPE),
                cudaHostAllocMapped);
  cudaHostAlloc((void **)&c, ((size_t)STREAM_ARRAY_SIZE) * sizeof(STREAM_TYPE),
                cudaHostAllocMapped);
...
#pragma omp atomic
  ...
  if (STREAM_ARRAY_SIZE % (k * 512) != 0) {
    printf("Please define STREAM_ARRAY_SIZE such that it is divisable by "
...
#pragma omp parallel for
  ...
  cudaMemcpy(ad, a, ((size_t)STREAM_ARRAY_SIZE) * sizeof(STREAM_TYPE),
             cudaMemcpyHostToDevice);
  cudaMemcpy(bd, b, ((size_t)STREAM_ARRAY_SIZE) * sizeof(STREAM_TYPE),
             cudaMemcpyHostToDevice);
  cudaMemcpy(cd, c, ((size_t)STREAM_ARRAY_SIZE) * sizeof(STREAM_TYPE),
             cudaMemcpyHostToDevice);
#endif
...
#endif
#if defined(USE_CUDA_DEVICE_ALLOC)
  cudaMemcpy(a, ad, ((size_t)STREAM_ARRAY_SIZE) * sizeof(STREAM_TYPE),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(b, bd, ((size_t)STREAM_ARRAY_SIZE) * sizeof(STREAM_TYPE),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(c, cd, ((size_t)STREAM_ARRAY_SIZE) * sizeof(STREAM_TYPE),
             cudaMemcpyDeviceToHost);
#endif
...
#pragma omp parallel
  {
...
#endif
#if defined(BENCH_AVX2) && defined(USE_FLOAT)
    for (j = start; j < start + chunk; j += 8)
      _mm256_stream_ps(&c[j], _mm256_load_ps(&a[j]));
#elif defined(BENCH_AVX2)
    for (j = start; j < start + chunk; j += 4)
      _mm256_stream_pd(&c[j], _mm256_load_pd(&a[j]));
#endif
    ...
    __asm__ __volatile__("mov x0, %0\n\t" // a
                         ...
                         "cbnz x4, 1b\n\t"
                         :
                         : "r"(&a[start]), "r"(&c[start]), "r"(chunk)
                         : "x0", "x1", "x4", "d0", "d1", "d2", "d3");
#endif
#ifndef BENCH_AVX512
#ifndef BENCH_AVX2
#ifndef BENCH_AVX
...
#endif
#if defined(BENCH_POWER8) && defined(USE_FLOAT)
  __attribute__((aligned(128))) STREAM_TYPE pumped_scalar[4] = {scalar, scalar,
                                                                scalar, scalar};
  vector float vecscalar =
      vec_vsx_ld(0, (const unsigned char *)pumped_scalar); // <— FIXED
...
#pragma omp parallel
  {
    ...
    for (j = start; j < start + chunk; j += 4)
      __riscv_vse32_v_f32m1(
          &b[j],
          __riscv_vfmul_vf_f32m1(__riscv_vle32_v_f32m1(&c[j], gvl), vecscalar,
                                 gvl),
          gvl);
#elif defined(BENCH_RV64)
    for (j = start; j < start + chunk; j += 2)
      __riscv_vse64_v_f64m1(
          &b[j],
          __riscv_vfmul_vf_f64m1(__riscv_vle64_v_f64m1(&c[j], gvl), vecscalar,
                                 gvl),
          gvl);
#endif
#if defined(BENCH_ARMV8) && defined(USE_FLOAT)
    ...
    __asm__ __volatile__(
        ...
        "cbnz x4, 1b\n\t"
        :
        : "r"(&b[start]), "r"(&c[start]), "r"(&scalar), "r"(chunk)
        : "x0", "x1", "x3", "x4", "d0", "d1", "d2", "d3", "d6");
#endif
...
#pragma omp parallel
  {
    ...
    for (j = start; j < start + chunk; j += 16)
      _mm512_stream_ps(
          &c[j], _mm512_add_ps(_mm512_load_ps(&a[j]), _mm512_load_ps(&b[j])));
#elif defined(BENCH_AVX512)
    for (j = start; j < start + chunk; j += 8)
      _mm512_stream_pd(
          &c[j], _mm512_add_pd(_mm512_load_pd(&a[j]), _mm512_load_pd(&b[j])));
#endif
#if defined(BENCH_AVX) && defined(USE_FLOAT)
    for (j = start; j < start + chunk; j += 8)
      _mm256_stream_ps(
          &c[j], _mm256_add_ps(_mm256_load_ps(&a[j]), _mm256_load_ps(&b[j])));
#elif defined(BENCH_AVX)
    for (j = start; j < start + chunk; j += 4)
      _mm256_stream_pd(
          &c[j], _mm256_add_pd(_mm256_load_pd(&a[j]), _mm256_load_pd(&b[j])));
#endif
#if defined(BENCH_AVX2) && defined(USE_FLOAT)
    for (j = start; j < start + chunk; j += 8)
      _mm256_stream_ps(
    ...
    for (j = start; j < start + chunk; j += 4)
      __riscv_vse32_v_f32m1(
          &c[j],
          __riscv_vfadd_vv_f32m1(__riscv_vle32_v_f32m1((&a[j]), gvl),
                                 __riscv_vle32_v_f32m1((&b[j]), gvl), gvl),
          gvl);
#elif defined(BENCH_RV64)
    size_t gvl = __riscv_vsetvl_e64m1(16);
    for (j = start; j < start + chunk; j += 2) {
      __riscv_vse64_v_f64m1(
          &c[j],
          __riscv_vfadd_vv_f64m1(__riscv_vle64_v_f64m1((&a[j]), gvl),
                                 __riscv_vle64_v_f64m1((&b[j]), gvl), gvl),
          gvl);
    }
#endif
#if defined(BENCH_ARMV8) && defined(USE_FLOAT)
    ...
    __asm__ __volatile__("mov x0, %0\n\t" // a
                         ...
                         "cbnz x4, 1b\n\t"
                         :
                         : "r"(&a[start]), "r"(&b[start]), "r"(&c[start]),
                           "r"(chunk)
                         : "x0", "x1", "x2", "x3", "x4", "d0", "d1", "d2", "d3",
                           "d4", "d5", "d6", "d7", "d8");
#endif
...
#pragma omp parallel
  {
    ...
    for (j = start; j < start + chunk; j += 16) {
      ...
      _mm512_stream_ps(
          &a[j],
          _mm512_add_ps(_mm512_load_ps(&b[j]),
                        _mm512_mul_ps(vecscalar, _mm512_load_ps(&c[j]))));
    }
#elif defined(BENCH_AVX512)
    for (j = start; j < start + chunk; j += 8) {
#ifdef USE_AVX512_PREFETCH
    ...
    for (j = start; j < start + chunk; j += 8)
      _mm256_stream_ps(
          &a[j],
          _mm256_add_ps(_mm256_load_ps(&b[j]),
                        _mm256_mul_ps(vecscalar, _mm256_load_ps(&c[j]))));
#elif defined(BENCH_AVX)
    for (j = start; j < start + chunk; j += 4)
      _mm256_stream_pd(
          &a[j],
    ...
    for (j = start; j < start + chunk; j += 4)
      _mm_stream_ps(&a[j],
                    _mm_add_ps(_mm_load_ps(&b[j]),
                               _mm_mul_ps(vecscalar, _mm_load_ps(&c[j]))));
#elif defined(BENCH_SSE)
    for (j = start; j < start + chunk; j += 2)
      _mm_stream_pd(&a[j],
...
#if defined(BENCH_POWER8) && defined(USE_FLOAT)
    for (j = start; j < start + chunk; j += 4)
      vec_vsx_st(vec_add(vec_vsx_ld(0, &b[j]),
                         vec_mul(vecscalar, vec_vsx_ld(0, &c[j]))),
                 0, &a[j]);
#elif defined(BENCH_POWER8) /* double precision */
    for (j = start; j < start + chunk; j += 2)
      vec_vsx_st(
          vec_madd(vec_vsx_ld(0, &c[j]), vecscalar, vec_vsx_ld(0, &b[j])), 0,
          &a[j]);
#endif
#if defined(BENCH_RV64) && defined(USE_FLOAT)
    for (j = start; j < start + chunk; j += 4)
      __riscv_vse32_v_f32m1(
          &a[j],
          __riscv_vfmadd_vf_f32m1(__riscv_vle32_v_f32m1(&c[j], gvl), vecscalar,
                                  __riscv_vle32_v_f32m1(&b[j], gvl), gvl),
          gvl);
#elif defined(BENCH_RV64)
    for (j = start; j < start + chunk; j += 2)
      __riscv_vse64_v_f64m1(
          &a[j],
          __riscv_vfmadd_vf_f64m1(__riscv_vle64_v_f64m1(&c[j], gvl), vecscalar,
                                  __riscv_vle64_v_f64m1(&b[j], gvl), gvl),
          gvl);
#endif
#if defined(BENCH_ARMV8) && defined(USE_FLOAT)
